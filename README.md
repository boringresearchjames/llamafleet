...
- [ ] **Compact VRAM bars in GPU column** — replace the raw `X MB / Y MB` text in the GPU column with a narrow inline progress bar so VRAM utilisation is scannable at a glance across many instances without horizontal scrolling.
- [ ] **Better first-run / no-models UX** — when no instances are running and no model paths are configured, show a friendly onboarding prompt rather than an empty table. Guide the user toward the launch form or the local-models scan.
- [ ] **Log viewer auto-refresh** — the per-instance log panel currently requires a manual reload. Add a toggle to tail the log in real time (e.g. polling every 2 s or streaming via SSE) so stdout/stderr from `llama-server` is visible without user action.
- [ ] **Clone/swap model action** — a one-click button on a running instance that pre-fills the launch form with the same profile (GPU mask, context, runtime args) but leaves the model path editable, so swapping a model onto the same GPU allocation takes seconds instead of re-entering all fields.
