...
const CONDITION_TYPES = [
  ...
  { value: 'toolNameContains',     label: 'Tool name contains',      hasValue: true, placeholder: 'e.g. execute_command' },
...
class LfOrchestrationPanel extends HTMLElement {
  ...
  async _load() {
    try {
      ...
      // Seed running instances from store in case it already has data
  ...
  _backendLabel(backend) {
    ...
    if (backend.type === 'local') return `local:<code>${this._esc(backend.model)}</code>`;
    if (backend.type === 'config') {
      const cfg = this._instanceConfigs.find(c => c.id === backend.configId);
      return `config:<code>${this._esc(cfg?.name || backend.configName || backend.configId)}</code>`;
    }
    const fb = this._frontierBackends.find(b => b.id === backend.backendId);
    return `frontier:<code>${this._esc(fb?.name || backend.backendId)}</code>`;
  }
  ...
  _renderRulesList() {
    ...
    list.querySelectorAll('.orch-rule-cond-op').forEach(sel => {
      sel.onchange = () => { const i = +sel.dataset.i; this._rules[i].conditions[0].op = sel.value; };
    });
    list.querySelectorAll('.orch-rule-cond-value').forEach(inp => {
      inp.oninput = () => { const i = +inp.dataset.i; this._rules[i].conditions[0].value = inp.value; };
    });
